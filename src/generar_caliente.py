...
Lógica de selección (basada en análisis del picks_log + performance):
  PRIORIDAD: posiciones calientes históricas del draw SIEMPRE van primero.
  El top3 (topq) se usa solo como relleno si faltan números.

  - Señal MUY ALTA  (>= 0.035) → posiciones calientes del draw (hasta 6) + top3 relleno
  - Señal ALTA      (>= 0.025) → posiciones calientes del draw (hasta 5) + top3 relleno
  - Señal MEDIA     (>= 0.015) → posiciones calientes del draw (hasta 4) + top3 relleno
  - Señal BAJA      (<  0.015) → posiciones calientes del draw (hasta 3) + top3 relleno
...
def seleccionar_numeros(pick: dict) -> dict:
    ...
    if nivel == "muy_alta":
        # Señal >= 0.035: todas las posiciones calientes del draw (hasta 6)
        limite = 6
        for pos in hot_pos[:limite]:
            idx = pos - 1
            if 0 <= idx < len(top12):
                numeros_calientes.append(top12[idx])
        # Rellenar con top3 si aún hay espacio
        for n in topq:
            if n not in numeros_calientes:
                numeros_calientes.append(n)

    elif nivel == "alta":
        # Señal >= 0.025: posiciones calientes confirmadas (hasta 5)
        limite = 5
        for pos in hot_pos[:limite]:
            idx = pos - 1
            if 0 <= idx < len(top12):
        ...
        # Señal >= 0.015: posiciones calientes primero (hasta 4), top3 como relleno
        for pos in hot_pos[:4]:
            idx = pos - 1
            if 0 <= idx < len(top12):
                numeros_calientes.append(top12[idx])
        # Rellenar con top3 hasta llegar a 4
        for n in topq:
            if n not in numeros_calientes:
                numeros_calientes.append(n)
        ...
        # Señal baja < 0.015: posiciones calientes del draw (hasta 3), top3 como relleno
        for pos in hot_pos[:3]:
            idx = pos - 1
            if 0 <= idx < len(top12):
                numeros_calientes.append(top12[idx])
        # Rellenar con top3 si no hay suficientes posiciones calientes
        for n in topq:
            if n not in numeros_calientes:
                numeros_calientes.append(n)
            if len(numeros_calientes) >= 3:
                break
