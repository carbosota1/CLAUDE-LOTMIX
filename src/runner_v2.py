"""
runner_v2.py — Copia paralela de runner.py para acumular data de backtest.
No modifica el historial ni envía Telegram real. Lee el mismo historial que
el sistema en producción (data/histories/*.xlsx) y escribe sus propias
predicciones en data_v2/ y outputs_v2/, con el detalle completo del ensemble
(top12_detail, hit_ranks, context) para poder recalibrar con evidencia real.
"""
...
from telegram import send_telegram as _send_telegram_real


def send_telegram(*args, **kwargs) -> None:
    """Silenciado en el runner paralelo (data_v2) — solo acumula data,
    no debe generar notificaciones duplicadas del sistema en producción."""
    print("[MUTE] Telegram silenciado en runner_v2 — mensaje no enviado.")
    return None
...
DATA_DIR = "data_v2"
HIST_DIR = os.path.join("data", "histories")  # mismo historial que el sistema viejo — solo lectura
STATE_PATH = os.path.join(DATA_DIR, "state_v2.json")
OUT_DIR = "outputs_v2"
...
def log_pick(payload: dict) -> None:
    ...
    row = {
        ...
        # detalle de las 4 capas del ensemble + score final por cada número
        # del top12 — necesario para recalibrar pesos con backtest real.
        "top12_detail": json.dumps(bp.get("top12_detail", []), ensure_ascii=False),
        "weekday": ctx.get("weekday"),
        "time_bucket": ctx.get("time_bucket", ""),
        "last_two_draws": json.dumps(ctx.get("last_two_draws", []), ensure_ascii=False),
        "min_signal_threshold": ctx.get("min_signal_threshold"),
        "min_a11_threshold": ctx.get("min_a11_threshold"),
...
def grade_picks_from_histories() -> None:
    ...
    for _, r in pending.iterrows():
        ...
        top12_detail = _parse_json_list(r.get("top12_detail", "[]"))

        # Rank (posición 1-12 dentro del top12) en el que cayó cada número
        # que sí salió sorteado. Esto reemplaza el análisis manual de
        # "posiciones más frecuentes" — queda calculado en cada fila.
        hit_ranks = [
            d.get("rank") for d in top12_detail
            if isinstance(d, dict) and d.get("num") in drawn
        ]
        ...
        perf_rows.append({
            ...
            "hit_ranks": json.dumps(hit_ranks, ensure_ascii=False),
            "pale_hits": _pale_hits(pales, drawn),
            "weekday": r.get("weekday", ""),
            "time_bucket": r.get("time_bucket", ""),
            "min_signal_threshold": r.get("min_signal_threshold", ""),
            "min_a11_threshold": r.get("min_a11_threshold", ""),
            "top12_detail": r.get("top12_detail", "[]"),
...
def analyze_target_and_maybe_notify(
    ...
    # -----------------------------------------------------------------------
    # Detalle por número — guarda las 4 capas del ensemble + el score final
    # de runner.py para cada número del top12, en el orden en que quedaron.
    # Esto es lo que falta hoy para poder recalibrar pesos/umbrales con un
    # backtest real en vez de umbrales sobre el ganador únicamente.
    # -----------------------------------------------------------------------
    rec_indexed = rec.set_index("num")
    # rec_hist trae el "score" original del ensemble de analyze.py; rec["score"]
    # ya fue sobrescrito con el re-score de runner.py (boosts/penalizaciones).
    # Guardamos ambos por separado para poder comparar y recalibrar después.
    rec_hist_norm = rec_hist.copy()
    rec_hist_norm["num"] = rec_hist_norm["num"].astype(str).map(_norm2)
    rec_hist_indexed = rec_hist_norm.set_index("num")

    top12_detail = []
    for rank, n in enumerate(top12, start=1):
        if n not in rec_indexed.index:
            continue
        row = rec_indexed.loc[n]
        if isinstance(row, pd.DataFrame):
            row = row.iloc[0]
        hrow = rec_hist_indexed.loc[n] if n in rec_hist_indexed.index else None
        if isinstance(hrow, pd.DataFrame):
            hrow = hrow.iloc[0]

        def _f(source, col, default=0.0):
            try:
                return float(source.get(col, default)) if source is not None else default
            except Exception:
                return default
        ...
        top12_detail.append({
            "num": n,
            "rank": rank,
            "final_score": _f(row, "score"),
            "signal": _f(row, "signal"),
            "a11": int(row.get("a11", 0)) if row is not None else 0,
            "ensemble_score": _f(hrow, "score"),
            "score_window": _f(hrow, "score_window"),
            "score_cond_mi": _f(hrow, "score_cond_mi"),
            "score_markov": _f(hrow, "score_markov"),
            "p_base": _f(hrow, "p_base"),
            "mi": _f(hrow, "mi"),
            "p_value": _f(hrow, "p_value", 1.0),
        })
    ...
    payload = {
        ...
        "best_play": {
            ...
            "best_a11": best_a11,
            "top12_detail": top12_detail,
            "context": {
                "weekday": target_weekday,
                "time_bucket": target_bucket,
                "last_two_draws": [sorted(s) for s in last_two],
                "min_signal_threshold": min_signal,
                "min_a11_threshold": min_a11,
            },
            "debug": {
...
def main() -> None:
    ...
    # -----------------------------------------------------------------------
    # runner_v2: NO actualiza el historial (scraping/backfill). Esa parte ya
    # la hace el sistema viejo sobre las mismas data/histories/*.xlsx. Si
    # runner_v2 se ejecuta DESPUÉS del viejo en el mismo workflow, el
    # historial ya está actualizado cuando este runner lee para generar picks.
    #
    # Para detectar "hay un resultado nuevo" sin volver a scrapear, se
    # compara contra el propio state_v2 (independiente del state.json viejo):
    # si el resultado ya está en el xlsx pero state_v2 aún no lo había visto,
    # se marca como "recién detectado" — dispara el mismo flujo de picks que
    # tendría runner.py con un update real.
    # -----------------------------------------------------------------------
    date_str_today = today_str()
    last_updates = state.setdefault("last_updates", {})
    for item in SCHEDULE:
        if not _is_due(item, now_rd()):
            continue
        key = f"{date_str_today}|{item['lottery']}|{item['draw']}"
        if last_updates.get(key) == "done":
            continue
        if _has_row_for_date(item["lottery"], item["draw"], date_str_today):
            last_updates[key] = "done"
    ...
    if picks_all:
        ...
        # picks.json — formato web: todos los picks del slot actual
        web_payload = {
            "generated_at": now_rd().isoformat(),
            "event_key": event_key,
            "date": today_str(),
            "picks": [
                {
                    "lottery":   p["best_play"]["lottery"],
                    "draw":      p["best_play"]["draw"],
                    "time_rd":   p["best_play"]["time_rd"],
                    "decision":  p["best_play"]["decision"],
                    "topq":      p["best_play"]["topq"],
                    "top12":     p["best_play"]["top12"],
                    "pales":     p["best_play"]["pales"],
                    "ok_alert":  p["best_play"]["ok_alert"],
                    "best_signal": p["best_play"]["best_signal"],
                    "best_a11":  p["best_play"]["best_a11"],
                }
                for p in picks_all
            ]
        }
        ...
        print("[OK] Escritos outputs_v2/picks_all.json y outputs_v2/picks.json")

        # runner_v2: se omite el paso de generar_caliente.py + Telegram —
        # ese paso es puramente de notificación (números calientes al chat),
        # no aporta a la data que queremos acumular para el backtest.
